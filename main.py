    ...
    def run(self):
        ...
        for sample_name, pairs in self.ui_data.items():
            ...
            for pair in pairs:
                ...
                sample_data.append({
                    'PointName': pair['PointName'], # 透传测试点位名
...
class SampleTab(QWidget):
    def __init__(self):
        ...
        self.table.setColumnWidth(0, 100) # 固定点位列宽
    ...
    def auto_fill_point_name(self, row, path):
        """【新增】智能提取点位名：切除末尾的 _IL, -1 等无用后缀"""
        base_name = Path(path).stem
        # 正则：匹配并移除结尾的 "-1", "_1", "-IL", "_RL" 等后缀
        clean_name = re.sub(r'[-_]([iI][lL]|[rR][lL]|\d+)$', '', base_name)
        
        # 仅当文本框仍是默认值（如 "点位X"）或者是空时，才进行自动覆盖
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        # 将历史出现过的独特规格都添加到下拉列表
        unique_specs = list(set(self.config_map.values()))
        self.combo_spec.addItems(unique_specs)
        ...
        self.btn_add_tab.setObjectName("ChartButton") # 采用新UI类
